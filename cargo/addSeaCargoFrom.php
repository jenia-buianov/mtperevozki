...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	$month = $explode[1];
	$year = $explode[2];
	if (is_valid_data($_POST['cont'])  and is_valid_data($_POST['port_import']) and is_valid_data($_POST['port_export'])   and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['cargo_type']) and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and isValidEmail($_POST['email'], false) and is_valid_data($_POST['telefon'][0])) {
		...
		$res = mysqli_query($db, "INSERT INTO movers_cargo (`name`,`export`,`export_city`,`import`,`import_city`,`phone`,`email`,`skype`,`icq`,`face`,`date`,`type`,`order_date`,`volume`,`source`,`by_admin`,`export_city_port`,`import_city_port`,`container_type`) VALUES ('$cargo_type','$export_country','$export_city','$import_country','$import_city','$telefone_number','$email','$skype','$icq','$fio','$date_export','$type_transport','$date','$cargo_size','$source','$by_admin','$export_port','$import_port','$cont') ") or die(mysqli_error($db));
		...
		if (LANG=='ru') $LINK.=". <a href='".HOME.LANG."/sea_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>Просмотреть транспорт ";
		if (LANG=='ro') $LINK.=". <a href='".HOME.LANG."/sea_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>Oferte de transport ";
		if (LANG=='en') $LINK.=". <a href='".HOME.LANG."/sea_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>View transport ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/sea_transport.php?import='.$import_country.'&export='.$export_country.'" /></head></html>';
...
<p><form action="<?php echo str_replace("&amp;","&",$_SERVER["REQUEST_URI"]).'#'.$ANOTHER; ?>" method="POST" name="<?php echo $ANOTHER;?>" id="<?php echo $ANOTHER;?>" onsubmit=" return sendForm('<?php echo $ANOTHER;?>','cargo_type,export_country,import_country,type_transport,cargo_size,fio,t0,email,dt_export,port_export,port_import,type','<?php if($mosConfig_lang == 'ru') {
...
?>!<?php if($mosConfig_lang == 'ru') { echo "Контактный телефон"; }
							...
							?>!<?php if($mosConfig_lang == 'ru') { echo "Порт доставки"; }
								elseif ($mosConfig_lang == 'ro') { echo "Portul de incarcare"; }
								elseif ($mosConfig_lang == 'en') { echo "Loading harbor"; }
							?>!<?php if($mosConfig_lang == 'ru') { echo "Тип контейнера"; }
...
<select type="select" name="cont" size="1"  class="form-control"  id="type" style="width:100%;padding: 3px 5px;font-size: 1em;">
 <?php if($mosConfig_lang == 'ru') { echo '<option value="">- Укажите тип контейнера -</option>';}
...
<td style="padding-bottom:8px;"><span id="email_span"><input type="text" name="email" value="<?php echo $_POST['email'] ?>" size="25" id="email"  class="form-control" style="padding: 3px 5px;font-size: 1em;height:auto" /></span></td>
</tr>
