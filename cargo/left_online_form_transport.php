...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['import_country']) and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['type_transport']) and is_valid_data($_POST['company']) and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and is_valid_data($_POST['telefon'][0])) {
		...
		if(LANG=='ru') $LINK = "Вы успешно добавили транспорт ";

		if(LANG=='ro') $LINK = "Dvs ati adaugat cu succes transport ";

		if(LANG=='en') $LINK = "You successfull added transport ";
		...
		if(LANG=='ru') $LINK.=". <a href='".HOME.LANG."/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?import=".$export_country."&export=".$import_country."' style='color:white'>Просмотреть грузы ";

		if(LANG=='ro') $LINK.=". <a href='".HOME.LANG."/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?import=".$export_country."&export=".$import_country."' style='color:white'>Oferte cu marfa ";

		if(LANG=='en') $LINK.=". <a href='".HOME.LANG."/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?import=".$export_country."&export=".$import_country."' style='color:white'>View cargo ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?import='.$export_country.'&export='.$import_country.'" /></head></html>';

		$succes_message= "<p class='alert alert-dismissible alert-success' id='".$_POST['form_name']."' style='text-align:center;display:block;margin-bottom:2em;margin-left:35%;width:30%;margin-top:70px;'>".$LINK."</a></p>";
		...
		//echo $succes_message;
			...
			$err_message = '<p class="alert alert-dismissible alert-danger">Заполните пожалуйста обязательные поля!</p>1= '.$_POST['import_country'].' <br>2= '.$_POST['import_city'].'<br>3= '.$_POST['export_country'].' <br>4= '.$_POST['export_city'].'<br>5= '.$_POST['date_export'].'<br>6= '.$_POST['type_transport'].'<br>7= '.$_POST['company'].'<br>8= '.$_POST['fio'].'<br>9= '.$_POST['email'].'<br>10= '.$_POST['cargo_size'].'<br>11= '.$_POST['cargo_type'].'<br>12= '.$number;
...
<select type="select" name="type_transport" size="1"  id="type_transport" style="width:100%;padding: 3px 5px;font-size: 1em;">
...
<div class="form-group">
        ...
        <div class='input-group date' id='date_export'>
                    ...
                    <input type='text' class="form-control" name="date_export" id="dt_export" value="<?php echo $_POST['date_export'] ?>" style="width:100%;padding: 3px 5px;font-size: 1em;height:auto"/>
...
<td style="padding-bottom:8px;"><span id="company_span"><input type="text" name="company" id="company" value="<?php echo $_POST['company'] ?>" style="height:auto;padding: 3px 5px;font-size: 1em;" /></span></td>
