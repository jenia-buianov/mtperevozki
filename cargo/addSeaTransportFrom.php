...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['port_import']) and is_valid_data($_POST['port_export'])   and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and is_valid_data($_POST['telefon'][0])) {
		...
		if(LANG=='ru') $LINK = "Вы успешно добавили морской транспорт ";

		if(LANG=='ro') $LINK = "Dvs ati adaugat cu succes transport maretian ";

		if(LANG=='en') $LINK = "You successfull added seafreight transport ";
		...
		if(LANG=='ru') $LINK.=". <a href='".HOME.LANG."/sea_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Просмотреть грузы ";

		if(LANG=='ro') $LINK.=". <a href='".HOME.LANG."/sea_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Oferte cu marfa ";

		if(LANG=='en') $LINK.=". <a href='".HOME.LANG."/sea_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>View cargo ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/sea_cargo.php?import='.$export_country.'&export='.$import_country.'" /></head></html>';
...
<p><form action="<?php echo str_replace("&amp;","&",$_SERVER["REQUEST_URI"]).'#'.$ANOTHER; ?>" method="POST" name="<?php echo $ANOTHER;?>" id="<?php echo $ANOTHER;?>" onsubmit=" return sendForm('<?php echo $ANOTHER;?>','export_country,import_country,cargo_size,dt_export,fio,t0,port_export,port_import','<?php if($mosConfig_lang == 'ru') { echo "Страна отправки"; }
...
?>!<?php if($mosConfig_lang == 'ru') {
 ...
 ?>!<?php
if($mosConfig_lang == 'ru') { echo "Свободен с/по"; }
...
?>!<?php if($mosConfig_lang == 'ru') { echo "Контактный телефон"; }
								...
								?>!<?php if($mosConfig_lang == 'ru') { echo "Порт отправки"; }
								elseif ($mosConfig_lang == 'ro') { echo "Portul de incarcare"; }
								elseif ($mosConfig_lang == 'en') { echo "Loading harbor"; }
							?>!<?php if($mosConfig_lang == 'ru') { echo "Порт доставки"; }
								elseif ($mosConfig_lang == 'ro') { echo "Portul de incarcare"; }
								elseif ($mosConfig_lang == 'en') { echo "Loading harbor"; }
							?>');">
...
<td style="padding-bottom:8px;"><span id="company_span"><input type="text" name="company" value="<?php echo $_POST['company'] ?>" id="company"  style="height:auto;padding: 3px 5px;font-size: 1em;" /></span></td>
...
<td style="padding-bottom:8px;"><span id="email_span"><input type="text" name="email" value="<?php echo $_POST['email'] ?>" size="25" style="padding: 3px 5px;font-size: 1em;height:auto" /></span></td>
