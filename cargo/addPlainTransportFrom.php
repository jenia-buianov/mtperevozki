...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['port_import']) and is_valid_data($_POST['port_export'])   and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and is_valid_data($_POST['telefon'][0])) {
		...
		$res = mysqli_query($db,"INSERT INTO movers_order(company,face,phone,email,type,volume,export,export_city,import,import_city,date_from,date_to,order_date,source,by_admin,export_city_port,import_city_port) VALUES ('$company','$fio','$telefone_number','$email','$type_transport','$cargo_size','$import_country','$import_city','$export_country','$export_city','$date_export','$date_export','$date','$source','$by_admin','$export_port','$import_port') ") or die(mysqli_error($db));
		...
		if(LANG=='ru') $LINK = "Вы успешно добавили авиа транспорт ";

		if(LANG=='ro') $LINK = "Dvs ati adaugat cu succes transport aerian ";

		if(LANG=='en') $LINK = "You successfull added plain transport ";
		...
		if(LANG=='ru') $LINK.=". <a href='".HOME.LANG."/plain_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Просмотреть грузы ";

		if(LANG=='ro') $LINK.=". <a href='".HOME.LANG."/plain_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Oferte cu marfa ";

		if(LANG=='en') $LINK.=". <a href='".HOME.LANG."/plain_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>View cargo ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/plain_cargo.php?import='.$export_country.'&export='.$import_country.'" /></head></html>';
...
<p><form action="<?php echo str_replace("&amp;","&",$_SERVER["REQUEST_URI"]).'#'.$ANOTHER; ?>" method="POST" name="<?php echo $ANOTHER;?>" id="<?php echo $ANOTHER;?>" onsubmit=" return sendForm('<?php echo $ANOTHER;?>','export_country,import_country,transport_type,dt_export,fio,t0,port_export,port_import','<?php if($mosConfig_lang == 'ru') { echo "Страна отправки"; }
...
?>!<?php if($mosConfig_lang == 'ru') {
 ...
 ?>!<?php
if($mosConfig_lang == 'ru') { echo "Свободен с"; }
...
?>!<?php if($mosConfig_lang == 'ru') { echo "Контактный телефон"; }
								...
								?>!<?php if($mosConfig_lang == 'ru') { echo "Аэропорт отправки"; }
								elseif ($mosConfig_lang == 'ro') { echo "Aeroportul de incarcare"; }
								elseif ($mosConfig_lang == 'en') { echo "Loanding airport"; }
							?>!<?php if($mosConfig_lang == 'ru') { echo "Аэропорт доставки"; }
								elseif ($mosConfig_lang == 'ro') { echo "Aeroportul de incarcare"; }
								elseif ($mosConfig_lang == 'en') { echo "Unloading airport"; }
							?>');">
...
<td align="left" style="padding-bottom:8px;">
<input type="text" class="form-control" id="port_import" value="<?php echo $_POST['port_import'] ?>" name="port_import" style="width:100%;padding: 3px 5px;font-size: 1em;height:auto"> 
</td>
</tr>
