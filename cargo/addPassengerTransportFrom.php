...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['transport_type']) and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['fio']) and is_valid_data($_POST['telefon'][0])) {
		...
		$res = mysqli_query($db,"INSERT INTO movers_order_passengers(company,face,phone,email,type,export,export_city,import,import_city,date,order_date,source,by_admin,passenger_number) VALUES ('$company','$fio','$telefone_number','$email','$type_transport','$import_country','$import_city','$export_country','$export_city','$date_export','$date_export','$source','$by_admin',$cargo_size) ") or die(mysqli_error($db));
		...
		if(LANG=='ru') $LINK = "Вы успешно добавили транспорт для пассажирских перевозок";

		if(LANG=='ro') $LINK = "Dvs ati adaugat cu succes transport pentru transportări ";

		if(LANG=='en') $LINK = "You successfull added passenger transport ";
		...
		if(LANG=='ru') $LINK.=". <a href='".HOME.LANG."/passenger_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Просмотреть грузы ";

		if(LANG=='ro') $LINK.=". <a href='".HOME.LANG."/passenger_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Oferte cu marfa ";

		if(LANG=='en') $LINK.=". <a href='".HOME.LANG."/passenger_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>View cargo ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/passenger_cargo.php?import='.$export_country.'&export='.$import_country.'" /></head></html>';
...
<p><form action="<?php echo str_replace("&amp;","&",$_SERVER["REQUEST_URI"]).'#'.$ANOTHER; ?>" method="POST" name="<?php echo $ANOTHER;?>" id="<?php echo $ANOTHER;?>" onsubmit=" return sendForm('<?php echo $ANOTHER;?>','export_country,import_country,transport_type,dt_export,fio,t0,cargo_size','<?php if($mosConfig_lang == 'ru') { echo "Страна отправки"; }
