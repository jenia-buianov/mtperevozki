...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['transport_type']) and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['cargo_type']) and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and isValidEmail($_POST['email'], false) and is_valid_data($_POST['telefon'][0])) {
		...
		$query = "INSERT INTO movers_cargo_post (`name`,`export`,`export_city`,`import`,`import_city`,`phone`,`email`,`skype`,`icq`,`face`,`type`,`order_date`,`volume`,`source`,`by_admin`) VALUES ('$cargo_type','$export_country','$export_city','$import_country','$import_city','$telefone_number','$email','$skype','$icq','$fio','$type_transport','$date_export','$cargo_size','$source','$by_admin') ";
		...
		if (LANG=='ru') $LINK.=". <a href='".HOME.LANG."/post_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>Просмотреть транспорт ";
		if (LANG=='ro') $LINK.=". <a href='".HOME.LANG."/post_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>Oferte de transport ";
		if (LANG=='en') $LINK.=". <a href='".HOME.LANG."/post_transport.php?import=".$import_country."&export=".$export_country."' style='color:white'>View transport ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/post_transport.php?import='.$import_country.'&export='.$export_country.'" /></head></html>';
...
<td style="padding-bottom:8px;"><span id="email_span"><input type="text" name="email" size="25" id="email" value="<?php echo $_POST['email'] ?>" class="form-control" style="padding: 3px 5px;font-size: 1em;height:auto" /></span></td>
</tr>
