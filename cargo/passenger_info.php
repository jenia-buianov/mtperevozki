...
if ($_GET) {
 ...
 if(isset($_GET["id"]) and isset($_GET['action']) and $_GET['action']=='link') {
	...
	$query="SELECT * FROM `movers_cargo_passengers` WHERE id='".$id."' LIMIT 1";
		...
		if (mysqli_num_rows($result) == 1) {
			...
			$getName = mysqli_query($db,"SELECT `cargo_type_$language` AS `n` FROM `cargo_type_passengers` WHERE `id`='$row[name]'");
			$name = mysqli_fetch_array($getName);
			
					
			$getTrasnport = mysqli_query($db,"SELECT `transport_type_$language` AS `t` FROM `transport_type_passengers` WHERE `id`='$row[type]'");
			...
			$table.=
				...
				"<div style='text-align:left;'><strong>&nbsp;Иформация о перевозке:</strong></div>
				...
				<table style='width:100%;'>
					...
					if($type_order) $table.="Дата отправки:";

					else $table.="Дата отправки:";
					...
					if ($type_order) $table.="Кол-во пассажиров:";

					else $table.="Кол-во пассажиров:";

					$table.="</td>

					<td>до ".$row['passenger_number']."</td>
