...
if (!defined('INSERTED')&&!empty($_POST)&&$_POST['form_name']==$ANOTHER) {
	...
	if (is_valid_data($_POST['cont']) and is_valid_data($_POST['export_country']) and is_valid_data($day)and is_valid_data($month)and is_valid_data($year) and mb_strlen($day,'UTF-8')==2 and mb_strlen($month,'UTF-8')==2  and mb_strlen($year,'UTF-8')==4 and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and is_valid_data($_POST['telefon'][0])) {
		...
		$res = mysqli_query($db,"INSERT INTO movers_order(company,face,phone,email,type,volume,export,import,date_from,date_to,order_date,source,by_admin,container_type) VALUES ('$company','$fio','$telefone_number','$email','$type_transport','$cargo_size','$import_country','$export_country','$date_export','$date_export','$date','$source','$by_admin','$cont') ") or die(mysqli_error($db));
		...
		if(LANG=='ru') $LINK = "Вы успешно добавили Ж.Д. транспорт ";

		if(LANG=='ro') $LINK = "Dvs ati adaugat cu succes transport feroviar ";

		if(LANG=='en') $LINK = "You successfull added rail transport ";
		...
		if(LANG=='ru') $LINK.=". <a href='".HOME.LANG."/train_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Просмотреть грузы ";

		if(LANG=='ro') $LINK.=". <a href='".HOME.LANG."/train_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>Oferte cu marfa ";

		if(LANG=='en') $LINK.=". <a href='".HOME.LANG."/train_cargo.php?import=".$export_country."&export=".$import_country."' style='color:white'>View cargo ";
		...
		echo '<html><head><meta http-equiv="refresh" content="4;URL='.HOME.LANG.'/train_cargo.php?import='.$export_country.'&export='.$import_country.'" /></head></html>';
...
<p><form action="<?php echo str_replace("&amp;","&",$_SERVER["REQUEST_URI"]).'#'.$ANOTHER; ?>" method="POST" name="<?php echo $ANOTHER;?>" id="<?php echo $ANOTHER;?>" onsubmit=" return sendForm('<?php echo $ANOTHER;?>','export_country,import_country,dt_export,fio,t0,port_export,port_import,type,cargo_size','<?php if($mosConfig_lang == 'ru') { echo "Страна отправки"; }
...
<td style="padding-bottom:8px;"><span id="fio_span"><input type="text" name="fio" size="25" value="<?php echo $_POST['fio'] ?>" id="fio"   style="height:auto;padding: 3px 5px;font-size: 1em;"/></span></td>
