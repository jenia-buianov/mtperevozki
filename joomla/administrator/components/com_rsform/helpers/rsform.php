				...
				foreach ($post as $key => $value)
				{
					if (is_array($value))
						foreach ($value as $post2 => $value2)
							$data[] = 'form['.urlencode($key).'][]='.urlencode($value2);
					else
						$data[] = 'form['.urlencode($key).']='.urlencode($value);
				}
				...
				else
				{
					...
					if ($silentPost->method)
					{
						...
						foreach ($post as $key => $value)
						{
							if (is_array($value))
								foreach ($value as $post2 => $value2)
									$output[] = '<input type="hidden" name="form['.RSFormProHelper::htmlEscape($key).'][]" value="'.RSFormProHelper::htmlEscape($value2).'" />';
							else
								$output[] = '<input type="hidden" name="form['.RSFormProHelper::htmlEscape($key).']" value="'.RSFormProHelper::htmlEscape($value).'" />';
						}
						$output[] = '</form>';
