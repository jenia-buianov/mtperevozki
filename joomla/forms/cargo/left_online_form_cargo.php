...
if ($_POST) {
	if (isset ($_GET['form']) && $_GET['form']=="left_order"){
		$date_export = mysql_real_escape_string($_POST['left_cargo_form_date_export']);
		
		if (is_valid_data($_POST['left_cargo_form_import_city']) and $_POST['left_cargo_form_import_city'] != "-1") {
			$import_city = mysql_real_escape_string($_POST['left_cargo_form_import_city']);
		} elseif (isset($_POST['left_cargo_form_import_city']) and $_POST['left_cargo_form_import_city'] == "-1" and is_valid_data($_POST['import_city_type'])) {
			$import_city = mysql_real_escape_string($_POST['import_city_type']);
		} else {
			$import_city = "";
		}

		if (is_valid_data($_POST['left_cargo_form_export_city']) and $_POST['left_cargo_form_export_city'] != "-1") {
			$export_city = mysql_real_escape_string($_POST['left_cargo_form_export_city']);
		} elseif (isset($_POST['left_cargo_form_export_city']) and $_POST['left_cargo_form_export_city'] == "-1" and is_valid_data($_POST['export_city_type'])) {
			$export_city = mysql_real_escape_string($_POST['export_city_type']);
		} else {
			$export_city = "";
		}
		
	}else if(isset ($_GET['form']) && $_GET['form']=="left_calc"){
		$date_export = mysql_real_escape_string($_POST['calculate_transportation_form_date_export']);
	...
	if (is_valid_data($_POST['import_country']) and is_valid_data($_POST['export_country']) and is_valid_data($date_export) and is_valid_data($_POST['type_transport']) and is_valid_data($_POST['cargo_type']) and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and isValidEmail($_POST['email'], false) and is_valid_telefone($_POST['telefon_country_code'], $_POST['telefon_city_code'], $_POST['telefon'])) {
...
<td align="left" style="padding-bottom:8px;">
<select type="select" name="export_city" size="1" id="export_city"  style="width: 220px" onchange="updateSelect('ignore', this.value, 'export_city');">
...
<td align="left" style="padding-bottom:8px;">
<select type="select" name="import_city" size="1" id="import_city" style="width: 220px" onchange="updateSelect('ignore', this.value, 'import_city');">
<option value="0"><?php if($mosConfig_lang == 'russian') {
	echo '---не имеет значения---';}
    elseif($mosConfig_lang == 'romanian') { echo'---oricare---';}
    elseif($mosConfig_lang == 'english') { echo'---all cities---';}
