...
if ($_GET) {
 if(isset($_GET["id"]) and isset($_GET['action']) and $_GET['action']=='link') {
	...
	if ($_POST['submit']) {
		...
		$res = mysql_query("UPDATE movers_cargo SET by_admin = $id_contact WHERE id=$id");
	...
	$contact_info = mysql_query("SELECT b.face, b.comment, a.by_admin, a.id FROM movers_cargo AS a LEFT JOIN movers_contact AS b ON a.by_admin = b.id WHERE a.id = '$id'");

	if (mysql_num_rows($contact_info) != 1) {
		$contact = "По умолчанию";
	} else {
		$row = mysql_fetch_array($contact_info);
		$contact = $row['face'];
	...
	$res = mysql_query("SELECT * FROM movers_contact WHERE id = 1") or die("Ошибка подключения");
	...
	$query="SELECT m.*, a.country_name_$language as export_title, a.alpha3 as export_code, b.country_name_$language as import_title, b.alpha3 as import_code,"
		...
		if (mysql_num_rows($result) == 1) {
			...
			$title .= $export." - ".$import.".";
			if($type_order and !empty($date)) $title.=" Свободен с ".$date.".";
			...
			$table.=
				...
				<table border='1' class='cargo_table' style='width:100%;'>
					...
					if($type_order) $table.="Свободен с:";
