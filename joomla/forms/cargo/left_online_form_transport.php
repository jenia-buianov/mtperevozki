...
if ($_POST) {
	...
	if (is_valid_data($_POST['import_country']) and is_valid_data($_POST['export_country']) and is_valid_data($_POST['calculate_transportation_form_date_export']) and is_valid_data($_POST['type_transport']) and is_valid_data($_POST['company']) and is_valid_data($_POST['cargo_size']) and is_valid_data($_POST['fio']) and isValidEmail($_POST['email'], false) and is_valid_telefone($_POST['telefon_country_code'], $_POST['telefon_city_code'], $_POST['telefon'])) {
		...
		if (is_valid_data($_POST['calculate_transportation_form_import_city']) and $_POST['calculate_transportation_form_import_city'] != "-1") {
			$import_city = mysql_real_escape_string($_POST['calculate_transportation_form_import_city']);
		} elseif (isset($_POST['calculate_transportation_form_import_city']) and $_POST['calculate_transportation_form_import_city'] == "-1" and is_valid_data($_POST['import_city_type'])) {
			$import_city = mysql_real_escape_string($_POST['import_city_type']);
		} else {
				$import_city = "";
		}

		if (is_valid_data($_POST['calculate_transportation_form_export_city']) and $_POST['calculate_transportation_form_export_city'] != "-1") {
			$export_city = mysql_real_escape_string($_POST['calculate_transportation_form_export_city']);
		} elseif (isset($_POST['calculate_transportation_form_export_city']) and $_POST['calculate_transportation_form_export_city'] == "-1" and is_valid_data($_POST['export_city_type'])) {
			$export_city = mysql_real_escape_string($_POST['export_city_type']);
		} else {
				$export_city = "";
		}

		if (is_valid_data($_POST['other'])) $description = mysql_real_escape_string($_POST['other']);
		...
		//$query = "INSERT INTO movers_order ('export','export_city','import','import_city','phone','email','face','free','type','order_date','volume','description') VALUES ('$cargo_type','$export_country','$export_city','$import_country','$import_city','$telefone_number','$email','$skype','$icq','$fio','$date_export','$type_transport','$date','$cargo_size')";

		$res = mysql_query("INSERT INTO movers_order(company,face,phone,email,type,volume,export,export_city,import,import_city,date_from,date_to,description,order_date,source,by_admin) VALUES ('$company','$fio','$telefone_number','$email','$type_transport','$cargo_size','$import_country','$import_city','$export_country','$export_city','$date_export','$date_export','$description','$date','$source','$by_admin') ") or die(mysql_error());
		...
		$succes_message ="<p>Для&nbsp; быстрого ответа на Ваш запрос&nbsp; обратитесь к менеджерам по направлениям.<br />
		<br />
		...
		header("Location:http://".$_SERVER['SERVER_NAME']."/index.php?option=com_content&id=7&task=view&Itemid=25&import=".$import_country."&export=".$export_country."&day=".$day."&month=".$month."&year=".$year);
		exit;
	} else {
			$succes_message ="";
...
function parse_info(form_user)
{
	...
	if (form_user.company.value == 0 || form_user.company.value == "" || form_user.company.value == null) {document.getElementById("company_span").className = "redborder"; err_message +="1";}
	else {document.getElementById("company_span").className = "none";}
...
<select type="select" name="import_country" size="1" onchange="updateSelect('import_city', this.value, 'import_city');" id="import_country" style="width: 220px">
...
<td align="left" style="padding-bottom:8px;">
<select type="select" name="import_city" size="1" id="import_city"  style="width: 175px" onchange="updateSelect('ignore', this.value, 'import_city');" style="width: 220px">
...
<select type="select" name="export_country" size="1" id="export_country" onchange="updateSelect('export_city', this.value, 'export_city');" style="width: 220px">
<?php if($mosConfig_lang == 'russian') { echo '<option value="">- укажите страну -</option>';}
      elseif($mosConfig_lang == 'romanian') { echo '<option value="">- alegeti tara  -</option>';}
      elseif($mosConfig_lang == 'english') { echo '<option value="">- choose the country -</option>';}
	  echo $countries;
...
<td align="left" style="padding-bottom:8px;">
<select type="select" name="export_city" size="1" id="export_city" style="width: 175px" onchange="updateSelect('ignore', this.value, 'export_city');" style="width: 220px" >
<option value="0"><?php if($mosConfig_lang == 'russian') {
	echo '---не имеет значения---';}
    elseif($mosConfig_lang == 'romanian') { echo'---oricare---';}
    elseif($mosConfig_lang == 'english') { echo'---all cities---';}
...
<tr>
<td style="padding-bottom:8px;">
Свободен с <span style="color: red">*</span>
</td>
<td style="padding-bottom:8px;">
...
<tr>
<td style="padding-bottom:8px;">
<?php if($mosConfig_lang == 'russian') { echo "Объём груза, вес"; }
		elseif ($mosConfig_lang == 'romanian') { echo "Volumul si capcitatea"; }
		elseif ($mosConfig_lang == 'english') { echo "Car volume and capacity"; }
?><span style="color: red">*</span>
</td>
<td style="padding-bottom:8px;">
...
<tr>
<td style="padding-bottom:8px;"><?php if($mosConfig_lang == 'russian') { echo "Компания"; }
	elseif ($mosConfig_lang == 'romanian') { echo "Compania"; }
	elseif ($mosConfig_lang == 'english') { echo "Company"; }
?><span style="color: red">*</span></td>
<td style="padding-bottom:8px;"><span id="company_span"><input type="text" name="company" size="25" id="company"  style="width: 220px" /></span></td>
</tr>
...
- <span id="telefon_span"><input type="text" name="telefon" size="15" maxlength="15" id="telefon" style="width: 130px" /></span></span>
</td>
...
<td style="padding-bottom:10px;"><textarea type="textarea" name="other" class="inputbox" cols="30" rows="5" id="other"  style="width: 220px" ></textarea></td>
</tr>
--->
<tr>
...
<input type="submit" name="submit" value="<?php if($mosConfig_lang == 'russian') { echo "Отправить"; }
