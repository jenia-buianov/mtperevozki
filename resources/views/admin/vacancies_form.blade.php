                ...
                @foreach($langs as $k=>$v)
                    <div class="tab-pane @if($k==0) active @endif" id="lang{{$v->code}}">
                        <div class="form-group">
                            ...
                            <input required type="text" class="form-control" name="title_{{$v->code}}" id="title" value="@if(isset($vac)){{__('vacancies.'.$vac->titleKey,[],$v->code)}}@endif">
                        </div>
                        <div class="form-group">
                            <label>{{__('admin.desc').' '.$v->title}}</label>
                            <textarea required name="text_{{$v->code}}" id="text_{{$v->code}}" rows="10" cols="80">@if(isset($vac)){{__('vacancies.'.$vac->textKey,[],$v->code)}}@endif</textarea>
                        </div>
                    </div>
                @endforeach
            </div>
